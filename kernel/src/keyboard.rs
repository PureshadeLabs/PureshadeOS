...
/// raw scan-code-set-2 bytes.  Wires GSI 1 (ISA IRQ 1) to `VECTOR_KBD`.
pub fn init() {
    flush_output();
    ...
    // Read config, clear translation, set port-1 IRQ enable.
    ctrl_cmd(CMD_READ_CFG);
    let cfg = match ctrl_read() {
        Some(b) => b,
        None    => return, // no i8042 present
    };
    ...
    // Wire ISA IRQ 1 (GSI 1, edge-triggered active-high) to our vector.
    crate::idt::register_irq(VECTOR_KBD, kbd_isr_stub as *const () as u64);
    crate::ioapic::map_irq(1, VECTOR_KBD, 0);
}
