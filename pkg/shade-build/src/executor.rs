...
mod host {
    ...
    use shade_store::{FsError, StoreFs};
    use shadec::io::EvalIo;

    use super::{clean_scratch, prepare_scratch, write_build_log};
    use crate::{path_str, plan_graph, BuildError, BuildPlan, Built, RecipeRef, Resolver};
    ...
    impl StoreRegistrar for DbRegistrar {
        fn register(&self, reg: &Registration) -> io::Result<()> {
            let out_path = reg.out_path.to_str().ok_or_else(|| {
                io::Error::new(io::ErrorKind::InvalidInput, "non-UTF-8 store path")
            })?;
            self.db
                .register(out_path, reg.digest, reg.store_name, reg.cdf_hash, reg.refs)
                .map(|_| ())
    ...
    impl ChildStream {
        fn create(store_name: &str, phase: usize) -> io::Result<ChildStream> {
            use core::sync::atomic::{AtomicU64, Ordering};
            static SEQ: AtomicU64 = AtomicU64::new(0);
            let path = std::env::temp_dir().join(format!(
                ".shade-build-phase-{}-{}-{store_name}-{phase}",
                std::process::id(),
    ...
    pub struct Executor<'a> {
        ...
        /// The B1 filesystem seam every executor-owned fs operation goes
        /// through: scratch, log, dep-existence, realization. `HostFs` by
        /// default; swapped by [`set_fs`](Executor::set_fs).
        fs: RefCell<Box<dyn StoreFs + 'a>>,
    }
    ...
    impl<'a> Executor<'a> {
        ...
        pub fn new(
            ...
            Executor {
                ...
                sandbox,
                registrar,
                fs: RefCell::new(Box::new(shade_store::HostFs)),
            }
        }

        /// Swap the filesystem backend (default [`HostFs`](shade_store::HostFs)).
        /// The sandbox and resolvers keep their own vehicles; tests inject the
        /// in-memory seam backend here, and on-target wiring injects
        /// [`OrosFs`](shade_store::OrosFs).
        pub fn set_fs(&mut self, fs: impl StoreFs + 'a) {
            self.fs = RefCell::new(Box::new(fs));
        }
        ...
        pub fn run(
            ...
            let graph = plan_graph(recipe, path_str(&self.store_root), toolchain, io)?;
                ...
                for dep_out in deps {
                    ...
                    if graph.closure.contains_key(&dep_key) {
                        ...
                        let store_name = dep_out
                            ...
                            .unwrap_or(&dep_out)
                            .to_string();
                        let realized = join(path_str(&self.store_root), &store_name);
                        if !self.fs.borrow_mut().exists(&realized) {
        ...
        fn parse_node(&self, drv_key: &str, cdf: &[u8]) -> Result<Node, BuildError> {
            ...
            let paths =
                shade_store::store_paths_at(path_str(&self.store_root), &name, &version, cdf)?;
        ...
        fn build_node(&self, node: &Node) -> Result<Built, BuildError> {
            ...
            let build_root = path_str(&self.build_root);
            let (scratch, staging) = {
                let mut fs = self.fs.borrow_mut();
                prepare_scratch(&mut **fs, build_root, store_name)
                    .map_err(seam_err("prepare_scratch", build_root))?
            };

            let result = self.build_in(node, Path::new(&scratch), Path::new(&staging));
            // Success or failure, the scratch (including any partial staged
            // outputs) is removed unless the caller asked to keep it.
            if !self.keep_failed {
                let mut fs = self.fs.borrow_mut();
                clean_scratch(&mut **fs, build_root, store_name);
            }
            result
        }

        /// Write the accumulated build log through the seam; returns its path.
        fn flush_log(&self, store_name: &str, buf: &[u8]) -> Result<String, BuildError> {
            let log_root = path_str(&self.log_root);
            let mut fs = self.fs.borrow_mut();
            write_build_log(&mut **fs, log_root, store_name, buf)
        ...
        fn build_in(
            ...
            // child output detours through a transient host file only because
            // spawn hands the builder a real fd (see ChildStream).
            ...
            for (i, phase) in node.phases.iter().enumerate() {
                ...
                let stream = ChildStream::create(store_name, i).map_err(BuildError::Io)?;
                let code = self.sandbox.spawn(&env, phase, stream.file()).map_err(BuildError::Io)?;
                logbuf.extend_from_slice(&stream.into_bytes().map_err(BuildError::Io)?);
                if code != 0 {
                    logbuf.extend_from_slice(
            ...
            let realized = {
                let mut fs = self.fs.borrow_mut();
                shade_store::realize_cdf(
                    &mut **fs,
                    path_str(&self.store_root),
                    &node.plan.name,
                    &node.plan.version,
                    &node.plan.cdf,
                    path_str(staging),
