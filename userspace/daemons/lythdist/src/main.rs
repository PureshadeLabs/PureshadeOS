...
use lythos_rt::{cap_rights, ipc::Endpoint, println, eprintln, sys_cap_grant, sys_task_exit};
...
fn panic(info: &core::panic::PanicInfo<'_>) -> ! {
    lythos_rt::sys_log("[lythdist] PANIC");
    ...
    if let Some(loc) = info.location() {
        ...
        if let Ok(s) = core::str::from_utf8(&buf[..n]) { lythos_rt::sys_log(s); }
        lythos_rt::sys_log("\n");
    } else {
        lythos_rt::sys_log("\n");
    }
