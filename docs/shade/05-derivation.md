...
the Canonical Derivation Form rpkg already defines. **CDF is not redefined
here** — it is [`rpkg 02 §3.2`](../rpkg/02-store.md#32-canonical-derivation-form-cdf);
its exhaustive key table is [`rpkg 02 §3.3`](../rpkg/02-store.md#33-hash-inputs).
...
set is closed ([`rpkg 02 §3.3`](../rpkg/02-store.md#33-hash-inputs): "adding
a key requires bumping `rpkg-drv`"). An unknown argument attribute is an
**eval error**, mirroring the TOML recipe unknown-key rule
([`rpkg 03`](../rpkg/03-recipe-format.md): "Unknown keys anywhere are a hard
error"). This keeps Shade and TOML emitting the *same* CDF surface
([`08 §2`](08-interop.md#2-interchangeability)).
...
| Attr | Type | Req | CDF target ([`rpkg 02 §3.3`](../rpkg/02-store.md#33-hash-inputs)) |
|---|---|---|---|
| `name` | string | yes | `name` (normalized per [`rpkg 02 §2`](../rpkg/02-store.md#2-store-path-format); non-normalizable = error, no guessing) |
| `version` | string | yes | `version` |
| `system` | string | yes | `system` |
| `toolchain` | string | yes* | `toolchain` (*may be omitted → defaults to the ambient toolchain identity string, [`rpkg 06 §4`](../rpkg/06-build.md#4-environment); `TODO(open):` once toolchain is a store dep, this becomes a `dep`, not a scalar — CDF v2, [`rpkg 06 §4`](../rpkg/06-build.md#4-environment)) |
| `sandbox` | int | no | `sandbox` (default `1`, the only profile, [`rpkg 06 §3.1`](../rpkg/06-build.md#31-contract-sandbox-profile-1)) |
| `sources` | list of source-specs | no | `source.<i>.*` (§4) |
| `deps` | list of derivations | no | `dep.<i>` — each element's `outPath` store path; sorted bytewise ([`rpkg 02 §3.3`](../rpkg/02-store.md#33-hash-inputs) ordering rule) |
| `env` | attrset (string→string) | no | `env.<KEY>` — keys must match `[A-Z_][A-Z0-9_]*`, values string-coerced; setting a sandbox-fixed var ([`rpkg 06 §4`](../rpkg/06-build.md#4-environment)) is an error |
| `phases` | list of strings | no | `phase.<i>` in list order (execution order) |
| `outputs` | attrset `{ bin=[…]; lib=[…]; share=[…]; }` | yes | `output.<i>` in `bin`,`lib`,`share` order then list order ([`rpkg 03 §6`](../rpkg/03-recipe-format.md#6-outputs)) |
| `unsafe` | bool | no | `unsafe=1` only when true ([`rpkg 03 §7`](../rpkg/03-recipe-format.md#7-unsafe-default-recipes)) |
| `description` | string | no | **not hashed** — carried on the value, dropped from CDF ([`rpkg 03 §2`](../rpkg/03-recipe-format.md#2-package)) |
| `license` | string | no | **not hashed** — same |
| `bootCritical` | bool | no | **not hashed** — carried to the generation manifest, not CDF ([`rpkg 03 §2`](../rpkg/03-recipe-format.md#2-package)) |
...
`rpkg-drv` (the format header line) is emitted by shadec unconditionally as
`1`, not an argument ([`rpkg 02 §3.2`](../rpkg/02-store.md#32-canonical-derivation-form-cdf)
rule 3).

### 2.1 `deps` — build vs runtime

CDF does not distinguish build from runtime deps — both are `dep.<i>`
([`rpkg 03 §4`](../rpkg/03-recipe-format.md#4-deps): "both kinds are hash
...
mirrors rpkg's `[deps].runtime`. Deferred with the rpkg sandbox fs work
([`rpkg 06 §3.2`](../rpkg/06-build.md#32-mechanism-on-oros)).
...
2. **Realize sources.** Each `sources` spec and each ingested path
   ...
   ([`rpkg 04 §2`](../rpkg/04-sources.md#2-source-derivations)); its store
...
4. **Build the key set** — populate every CDF key from the argument
   ...
   exclusions.
5. **Canonicalize** — sort keys bytewise ascending except the `rpkg-drv`
   header first; index the repeated lists (`source.<i>`, `dep.<i>`,
   `env.<KEY>`, `phase.<i>`, `output.<i>`) per the ordering rules
   ([`rpkg 02 §3.3`](../rpkg/02-store.md#33-hash-inputs)); percent-escape
   values ([`rpkg 02 §3.2`](../rpkg/02-store.md#32-canonical-derivation-form-cdf)
   rule 4); emit `key=value\n` lines with a trailing LF.
6. **Hash** — `BLAKE3(CDF bytes)`, truncate to 160 bits, base32-encode →
   the digest ([`rpkg 02 §3.1`](../rpkg/02-store.md#31-hash-function)).
   `drvPath = /r/store/<digest>-<name>-<version>.drv`;
   `outPath` = same without `.drv`.
7. **Hand off** — shadec writes the `.drv` (CDF bytes) into the store via
   the store services (not directly, [`08 §4`](08-interop.md#4-pipeline-integration)),
   exactly as rpkg's TOML compiler does ([`rpkg 03 §8`](../rpkg/03-recipe-format.md#8-recipe--derivation-compilation-summary)
   step 4). If `/r/db/valid/<digest>` exists, no rebuild.

Because steps 4–6 are the *same* canonicalization rpkg's TOML path uses,
**a Shade derivation and a TOML recipe that reduce to the same key set
produce byte-identical CDF and therefore the identical store path**
([`08 §2`](08-interop.md#2-interchangeability)). This is the load-bearing
...
rpkg ([`rpkg 03 §5.2`](../rpkg/03-recipe-format.md#52-substitution-variables)).
...
- **an ingested path** — a Shade path value; realized as a `local` source
  ...
  ([`rpkg 04 §3.3`](../rpkg/04-sources.md#33-local)).
...
- **an explicit source attrset** — a literal
  ...
  [`rpkg 04 §3`](../rpkg/04-sources.md#3-resolution-per-source-type). shadec
  ...
  **not** re-resolve — an explicit attrset is a *pinned* identity, the same
  values a lockfile would hold ([`rpkg 04 §5`](../rpkg/04-sources.md#5-lockfile)).

Source order in the list fixes `source.<i>` indices and the `$src<i>`
variables ([`rpkg 03 §3`](../rpkg/03-recipe-format.md#3-source--array-of-tables)).
Ordering rule: `source.*` in list order (recipe order), matching rpkg
([`rpkg 02 §3.3`](../rpkg/02-store.md#33-hash-inputs)).

### 4.1 Source derivations are trimmed CDFs

A source derivation omits `system`/`toolchain` and carries `builder=fetch`
([`rpkg 04 §2`](../rpkg/04-sources.md#2-source-derivations)). shadec emits
...
`builder`-key `TODO(open)` in rpkg 04 §2 applies unchanged; shadec follows
whatever rpkg freezes.
...
| Builtin | Signature | Emits (`source.*`) | rpkg source type |
|---|---|---|---|
| `builtins.fetchCratesIo` | `{ crate, version, sha256 }` → drv | `type=crates-io`, `crate`, `version`, `sha256` | [`rpkg 04 §3.1`](../rpkg/04-sources.md#31-crates-io) |
| `builtins.fetchGit` | `{ url, commit, submodules ? false }` → drv | `type=git`, `commit`, `submodules?` | [`rpkg 04 §3.2`](../rpkg/04-sources.md#32-git) |
| `builtins.fetchTree` | `{ url, type, ... , narHash }` → drv | generalized; `TODO(open)` below | — |
| `builtins.path` | `{ path, name ?, filter ? , sha256 ? }` → drv | `type=local`, `tree` | [`rpkg 04 §3.3`](../rpkg/04-sources.md#33-local) |
...
- `commit`/`sha256`/`tree` values are the **pinned** identities — shadec
  ...
  step 2) and fails closed on mismatch, exactly as rpkg fetch does
  ([`rpkg 04 §2`](../rpkg/04-sources.md#2-source-derivations)). The `url`
  is **not** hashed ([`rpkg 04 §3.2`](../rpkg/04-sources.md#32-git)); it is
  transport only.
...
rpkg source type; the generic form is speced as tier-3 and its exact
...
fetchers that emit non-rpkg source identities.
...
([`rpkg 03 §6`](../rpkg/03-recipe-format.md#6-outputs)), **not** Nix-style
separate `$out`/`$dev`/`$doc` store paths. This matches rpkg's store model
([`rpkg 02 §4`](../rpkg/02-store.md#4-store-contents): one output directory
with `bin/ lib/ share/`).

`TODO(open):` multi-output derivations (separate `dev`/`doc` store paths)
are a Nix feature with no rpkg store equivalent yet — adding them requires
rpkg 02/03 to grow multi-output store paths first. Deferred; Shade will not
lead rpkg here. `outputName`/`outputs`-as-list attributes are reserved on
...
[`rpkg 02 §3.3`](../rpkg/02-store.md#33-hash-inputs) as a Shade expression:
...
[`rpkg 02 §3.3`](../rpkg/02-store.md#33-hash-inputs) (given the same
`lythos-libstd` store path), hence the identical digest and store path —
the interop guarantee ([`08 §2`](08-interop.md#2-interchangeability)) made
concrete.
