...
pub fn map_pages(virt: u64, npages: usize) -> Result<(), lythos_std::SysError> {
    ...
    for i in 0..npages as u64 {
        lythos_std::sys_mmap(virt + i * 4096, 0, RW_USER)?;
    }
    Ok(())
}

/// Unmap a previously mapped page.
pub fn unmap_page(virt: u64) -> Result<(), lythos_std::SysError> {
    lythos_std::sys_munmap(virt)
}
